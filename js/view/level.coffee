define ['util/trigger', 'util/dom', 'util/matrix', 'util/async', 'util/constants'], (Trigger, dom, matrix, async, constants) ->
    ...
    class LevelView extends Trigger
        ...
        dropPlayer: (@player) ->
            @elem.appendChild player.view.elem
            player.on 'move', @onPlayerMove.bind @
        onPlayerMove: (playerModel, x, y) ->
            [transformed_x, transformed_y] = @displayTransform playerModel
            styles =
                left: dom.px transformed_x
                top: dom.px transformed_y
            dom.style @player.view.elem, styles
